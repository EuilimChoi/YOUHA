...
# 개발 내용
### ERD
- 하나의 영화는 여러개의 배우, 감독, 장르를 가질 수 있어 다대다 관계를 가지도록 설계 했습니다. 
- 트레일러일 경우 하나의 트레일러를 여러 영화에서 사용하는 경우는 없을 것으로 생각되어 다대일 관계로 작성했습니다.
...
- 영화 제목은 여러개를 넣어도 최초의 1개만 인식하도록 제작했습니다.
- 배우, 감독, 장르는 여러개를 지원합니다.

### 더미 데이터 업로드
- 실제 영화 내용을 기반으로 10개의 영화를 업로드해 두었습니다. 해당 데이터를 기반으로 로컬 테스트를 진행하시면 될 것 같습니다. 실제 데이터는 testresource.json 파일을 참해 주세요.
